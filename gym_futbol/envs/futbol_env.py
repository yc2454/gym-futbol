...
class FutbolEnv(gym.Env):
      ...
      def __init__(self, length = FIELD_LEN, width = FIELD_WID, goal_size = GOAL_SIZE, 
            ...
            self.observation_space = spaces.Box(low=np.array([[0, 0, -length, -width, 0]] * 6),
                                                high=np.array([[length, width, length, width, player_speed + 5],
                                                      [length, width, length, width, player_speed + 5],
                                                      [length, width, length, width, player_speed + 5],
                                                      [length, width, length, width, player_speed + 5],
                                                      [length, width, length, width, shoot_speed + 5],
                                                      [10, 10, 10, 10, 10]]),
                                                dtype=np.float64)
...
#            plt.show()
      ...
      def _set_vector_observation(self, agent, action_type, set_target = False, target = [0, 0]): 
            ...
            if agent.name == 'opp_1':
                  mate = self.opp_2
                  mate_index = self.opp_2_index
            elif agent.name == 'opp_2':
                  mate = self.opp_1
                  mate_index = self.opp_1_index
            elif agent.name == 'ai_1':
                  mate = self.ai_2
                  mate_index = self.ai_2_index
            else:
                  mate = self.ai_1
                  mate_index = self.ai_1_index 
            ...
            # when the agent doesn't have ball
                  ...
                  if action == Action.intercept: 
                        ...
                        if intercept_success or \
                              (self.ball_owner == BallOwner.NOONE and ball_to_agent_magnitude < MAX_INTERCEPT_DIST + 2): 
                  ...
                  # otherwise, run towards the goal
                      ...
                      # ball owener not change
                        ...
                        if set_target:
                              ...
                              if self.ball_owner != BallOwner(mate_index):
      ...
      def _get_reward(self, ball, ai_1, ai_2, opp_1, opp_2, ball_owner, actions):
            ...
            if ball_owner[self.ai_1_index] == 10:
                  if action2 == Action.run:
                        strategic_run_r_2 = STRATEGIC_RUN_BASE
                  else:
                        strategic_run_r_2 = 0
            else:
                  strategic_run_r_2 = 0

            if ball_owner[self.ai_2_index] == 10:
                  if action1 == Action.run:
                        strategic_run_r_1 = STRATEGIC_RUN_BASE
                  else:
                        strategic_run_r_1 = 0
            else:
                  strategic_run_r_1 = 0

            strategic_run_r = strategic_run_r_1 + strategic_run_r_2
