...
class Easy_Agent():
    ...
    def get_action_type(self, observations, has_ball, team_has_ball):
        ...
        _, ball_to_agent_magnitude = get_vec(
            self.ball[:2], self.agent_observation[:2])
        _, mate_to_agent_magnitude = get_vec(
            self.mate_observation[:2], self.agent_observation[:2])
